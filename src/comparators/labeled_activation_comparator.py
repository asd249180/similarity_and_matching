...
from src.bin.find_transform import get_hits_overlap
...
class LabeledActivationComparator(ActivationComparator):
    ...
    def _calculate_measures(self, measure_names):
        ...
        for measure_name in measure_names:
            labeled_activations = {mode : [activations[0][hit_masks[mode]], activations[1][hit_masks[mode]]] for mode in hit_masks}
            measure_function = get_comparator_function(measure_name)
            labeled_measurements = {label : measure_function(*labeled_activations[label]) for label in labeled_activations}
