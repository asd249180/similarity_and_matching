...
settings.mean_std_table.column_names = ['Cross-entropy', 'Accuracy'] # Order cannot be changed this way
...
def parse_args(args):
    ...
    parser.add_argument('--csv', default="results/official/tiny_abn/summary.csv")
    parser.add_argument('--out-dir', default='results/official/plots/tiny_abn/')
...
def save_diagram(df, out_dir):
    ...
    ax.set_xticklabels([f"{x.get_text().replace('Bn', 'Layer')}  " for x in ax.get_xticklabels()])
    ax.set_ylim([0,1])
