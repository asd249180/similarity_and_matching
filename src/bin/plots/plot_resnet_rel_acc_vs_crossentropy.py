...
def parse_args(args):
    ...
    parser.add_argument('--out-dir', default='results/official/plots/resnet20_w1_bn/')
...
def get_df(csv, out_dir):
    ...
    filtered_df['front_layer'] = filtered_df['front_layer'].str.replace("Bn1", "Layer0.0")
    ...
    df['ps_inv_acc'] = df.ps_inv_acc / df.end_acc  #df.ps_inv_same_class_out  #df.ps_inv_same_class_out # (df.m2_ps_inv_rr+df.m2_ps_inv_ww) / n
...
def save_diagram(df, out_dir):
    ...
    ax.set_xticklabels([x.get_text().replace('.add', '').replace('Bn1', 'Layer0') for x in ax.get_xticklabels()])
    ax.set_ylim([0,1])
