...
def parse_args(args):
    ...
    parser.add_argument('--csv', default="results/official/tiny_bn/summary.csv")
    parser.add_argument('--out-dir', default='results/official/plots/tiny_bn_cka_vs_ce/')
...
def save_diagram(df, out_dir):
    ...
    xlabels = df['front_layer'].drop_duplicates().str.replace('Bn', 'Layer').tolist()
    xlabels.sort()
    ...
    # g._legend.remove()
    legends = plt.legend(h,l,loc=1, fontsize=20)
    for i in range(4):
