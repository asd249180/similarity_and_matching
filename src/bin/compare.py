...
def _parse_args(args):
    ...
    parser.add_argument('dataset', help='Name of dataset', choices=['fashion', 'mnist', 'cifar10', 'cifar100', 'celeba'])
    parser.add_argument('methods', help='Name of second model', nargs="+", choices=['cka', 'l2', 'ps_inv', 'corr'])
    ...
    parser.add_argument('-t','--train-data', help='Run on training data', action='store_true')
...
def run_comparison_by_conf(conf, verbose=False):
    ...
    dataset_type = 'train' if conf.train_data else 'val'
    measures = comparator(
        conf.dataset,
        conf.methods,
